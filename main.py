...
def replace_letter(letter, n):
    ...
    index = ((alphabet.find(letter.lower())) + n) % len(
        alphabet)  # find letter in aplphabet (alphabet is variable defined above function - but can be inside)
    ...
    else:  # for everything that is not a letter - whiteholders, numbers, !@#$%^&*()_+ etc.
...
def decode(string, frequent_letter):
    ...
    frequencies = []  # list with frequencies of freuent letter in each translation
